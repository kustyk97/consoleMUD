...
class TestGame(unittest.TestCase):
    ...
    def test_write_message(self, mock_print):
        ...
        self.assertTrue(
            any("Insert message:" in message for message in printed_messages)
        )
    ...
    def test_choose_npc_valid(self, mock_print):
        ...
        for input_value in input_values:
            with patch("builtins.input", return_value=input_value):
                self.game.choose_npc(
                    "Which NPC do you want to fight?", [0, 0], self.game.start_fight
                )
                self.assertFalse(
                    mock_print.call_args_list
                    and any(
                        "Unknow number" in call[0][0]
                ...
                self.assertFalse(
                    mock_print.call_args_list
                    and any(
                        "Return to main menu" in call[0][0]
                        for call in mock_print.call_args_list
                    )
                )
